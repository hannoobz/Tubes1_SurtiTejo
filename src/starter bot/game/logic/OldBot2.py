...
class OldBot2(BaseLogic):
    ...
    def findNearestDiamond(self, closest_blue_diamond , closest_red_diamond, board_bot, board):
        ...
        if closest_blue_diamond and (not closest_red_diamond or OldBot2.calculate_distance(closest_blue_diamond.position, curr_position) < OldBot2.calculate_distance(closest_red_diamond.position, curr_position) * 0.75):
        ...
        if props.diamonds >= 4 and OldBot2.calculate_distance(self.target_position, base) >= 0.5 * (board.width + board.height):
    ...
    def make_decision(self):
        ...
        if (closest_enemy is not None and (closest_enemy_distance  <= 1 and closest_enemy.properties.diamonds >= 3)):
            ...
            if props.diamonds == 5 or (props.diamonds >=4 and OldBot2.calculate_distance(curr_position,props.base) >= 0.5*(self.board.width + self.board.height)) or (props.diamonds >= 3 and self.TimeLeft <= 15000) or (props.diamonds >= 2 and self.TimeLeft <= 10000) :
    ...
    def get_direction(self,current_x, current_y, dest_x, dest_y,  width, height):
        ...
        if (delta_x == 0 and delta_y == 0):
            return self.next_move(self.ownBot,self.board)
    ...
    def sortDiamonds(self,allDiamonds : List[GameObject], curr_position : Position):
        ...
        for diamond in allDiamonds:
            #blue diamond
            distance = OldBot2.calculate_distance(diamond.position,curr_position)
